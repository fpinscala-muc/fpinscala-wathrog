...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @annotation.tailrec
    def fib_int(acc: Int, temp: Int, n: Int): Int = {
      if (n <= 0) acc else fib_int(temp, acc+temp, n-1)
    }
    fib_int(0,1,n)
  }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    @annotation.tailrec
    def go(ahead:A, atail:Array[A], isSorted: Boolean): Boolean = {
      if(!isSorted || atail.length <= 0) isSorted else go(atail.head, atail.tail, gt(ahead,atail(0)))
    }
